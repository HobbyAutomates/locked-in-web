...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({}))) as { image?: string; media_type?: string; note?: string; text?: string; lens?: string };
  try {
    return NextResponse.json(await labelFlow({ admin, userId: user.id, image: body.image, mediaType: body.media_type, text: body.text, note: body.note, lens: body.lens }));
  } catch (e) {
