      ...
      {step === "username" ? (
        <div className="pb-1">
          <div className="relative">
            ...
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2" style={{ color: status === "ok" ? "var(--green)" : status === "taken" || status === "invalid" ? "var(--danger)" : "var(--muted)" }}>
              {status === "checking" ? <Spinner size={16} /> : status === "ok" ? <Check size={18} /> : status === "taken" || status === "invalid" ? <Close size={16} /> : null}
            </span>
          </div>
          <p className="mt-1.5 px-1 text-[12px]" style={{ color: status === "taken" || status === "invalid" ? "var(--danger)" : status === "ok" ? "var(--green)" : "var(--muted)" }}>
        ...
        <div className="flex flex-col items-center pb-1">
          ...
          <button type="button" className="press mt-4 rounded-full px-4 py-2 text-[13px] font-semibold" style={{ background: "var(--card2)", color: "var(--danger)", border: 0 }} onClick={() => setStep("photo")}>
