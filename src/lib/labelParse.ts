...
/**
 * Whether a structured label_report's nutrition can be trusted enough to cache (the barcode
 * `__reports` cache in barcode_cache.product): either its per_100g passes the sanity gate, or it
 * carries no numbers at all and says so with `needs_back_of_pack: true`. Anything else — numbers
 * that fail the gate, or numbers with no gate verdict — must never be cached.
 */
export function reportNutritionTrusted(report: Record<string, unknown> | null | undefined): boolean {
  if (!report) return false;
  if (report.needs_back_of_pack === true) return report.per_100g == null;
  const per100 = report.per_100g;
  if (!per100 || typeof per100 !== "object") return false;
  return sanityCheckPer100(per100 as Per100).ok;
}
